...
def visualize_images(fig, mag, kspace_db, title, save_path=None, losses=None, mag_target=None):
    ...
    nrows = 3  # Three rows: output magnitude, k-space, target magnitude
    gap = 0.15
    title_h = 0.4
    loss_h = 0.1
    cbar_w = 0.08
    ...
    right_margin = 0.5
    fig_w = ncols * img_w + (ncols - 1) * gap + 3 * cbar_w + 3 * cbar_gap + left_margin + right_margin  # 4 * 1.2 + 3 * 0.15 + 3 * 0.08 + 3 * 0.03 + 0.7 + 0.5 = 6.79
    fig_h = nrows * img_h + (nrows - 1) * gap + title_h + loss_h  # 3 * 2.4 + 2 * 0.15 + 0.4 + 0.1 = 8.0

    fig.set_size_inches(fig_w, fig_h)  # (6.79, 8.0) inches for screen fit
    ...
    for i in range(ncols):
        ...
        if i == ncols - 1:
            cbar_ax = fig.add_axes([(ncols * (img_w + gap) + left_margin + cbar_gap) / fig_w, (fig_h - img_h - title_h - loss_h) / fig_h, cbar_w / fig_w, img_h / fig_h])
        ...
        if i == ncols - 1:
            cbar_ax = fig.add_axes([(ncols * (img_w + gap) + left_margin + cbar_gap) / fig_w, (fig_h - 2 * img_h - gap - title_h - loss_h) / fig_h, cbar_w / fig_w, img_h / fig_h])
            fig.colorbar(im, cax=cbar_ax)

        # Target magnitude (third row)
        if mag_target is not None:
            ...
            if i == ncols - 1:
                cbar_ax = fig.add_axes([(ncols * (img_w + gap) + left_margin + cbar_gap) / fig_w, (fig_h - 3 * img_h - 2 * gap - title_h - loss_h) / fig_h, cbar_w / fig_w, img_h / fig_h])
                fig.colorbar(im, cax=cbar_ax)
